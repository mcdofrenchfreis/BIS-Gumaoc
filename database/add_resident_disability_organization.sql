...
ADD COLUMN IF NOT EXISTS `resident_disability` varchar(255) DEFAULT NULL AFTER `interviewer_title`;

-- Add organization column if it doesn't exist  
ALTER TABLE `resident_registrations` 
ADD COLUMN IF NOT EXISTS `resident_organization` varchar(255) DEFAULT NULL AFTER `resident_disability`;
